...
public class AddEmployee extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			int r = eBiz.add(e);
			if (r > 0) {
				out.println("<script>alert('添加成功');location.href='addEmp.jsp';</script>");
			} else {
				out.println("<script>alert('添加失败');location.href='addEmp.jsp';</script>");
			}

		} catch (ParseException e1) {
			// e1.printStackTrace();
			out.println("<script>alert('添加失败');locationhref='addEmp.jsp';</script>");
		}
		out.println("  </BODY>");
		out.println("</HTML>");
		out.flush();
